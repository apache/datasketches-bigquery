...
// no \0 termination
static inline void b64_encode(const char* src, unsigned srclen, char* dst) {
  unsigned buf = 0;
  int pos = 2;
...
static inline void b64_decode(const char* src, unsigned srclen, char* dst) {
  unsigned buf = 0;
  char c;
  int bits = 0;
  int pos = 0;
...
// with padding
static inline unsigned b64_enc_len(unsigned srclen) {
  return ((srclen + 2) / 3) * 4;
}

static inline unsigned b64_dec_len(const char* src, unsigned srclen) {
  unsigned pad = 0;
